...
import { applyMigration, scanMigration } from '../src/core/migrate.js';
import { createSkill, makeTempDir, writeFile } from './helpers.js';
...
test('migration wizard copies selected items, backs up, and links', async () => {
  ...
  const backup = await createBackupSession({
    canonicalRoot: path.join(home, '.agents'),
    scope: 'global',
    operation: 'test',
  });
